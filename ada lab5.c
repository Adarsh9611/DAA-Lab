...
printf("enter the number opf vertices/nodes in the graph\n");
scanf("%d",&n);
printf("enter the cost/weight matrix\n");
for(i=1;i<=n;i++){
parent[i]=0;
for(j=1;j<=n;j++){‘size_t’
/usr/include/stdlib.h:761:34: error: unkno
scanf("%d",&cost[i][j]);
if(cost[i][j]==0)

cost[i][j]=INF;
}}
printf("thge edges of minimun spanning tree are\n");
while(ne<n)
{
min=INF;
for(i=1;i<=n;i++){
